...
if (isset($_COOKIE['trolley'])){
  ...
  foreach ($trolley as $key => $value) {
    ...
    <div class="cantidad">
      ...
      <button class="change_button" type="button" onclick="mostrarInput(' . $value[0] . ')">Cambiar</button>
      <div  id="inputCantidad' . $value[0] . '" style="display:none">
        ...
        <button class="change_button" type="submit" name="actualizar_cantidad" value="' . $value[0] . '">Actualizar</button>
      </div>
...
if (isset($_POST["finalizar_transaccion"])) {
  ...
  foreach ($trolley as $producto) {
    ...
    $stock = $sh->select_values("PRECIO", "Cantidad_disponible", "WHERE ID_producto = {$id_producto}");
    $stock = $stock[0]["Cantidad_disponible"];
    if ($cantidad > $stock){
      echo '<script>alert("No hay suficiente stock de ' . $producto[2] . '")</script>';
    } else {
